      ...
      <div className="card" style={{ width: '43rem', height: '40rem' }}>
        <Tabs defaultActiveKey="tabFirst" id="uncontrolled-tab-example">
          <Tab eventKey="tabFirst" title="Reseptin etusivu">
            <img
              src={`http://localhost:8081/images/${details.photos}`}
              alt={details.title}
              style={{ width: '50%', height: 'auto' }}
            />
            <h5 className="card-title">{details.title}</h5>
            <p>Julkaisija: {details.author_nickname}</p>
            <p>Resepti luotu: {details.created_at}</p>
            <p>Näkyvyys: {details.visibility === 1 ? 'Julkinen' : 'Vain jäsenille'}</p>
            <p>Käyttäjien antama arvosana: {details.average_rating}</p>
          </Tab>
